...
export const AuraCvxDepositPanel: FC<AuraCvxDepositPanelProps> = ({amounts, setAmount}) => {
  ...
  return (
    <Flex direction={"column"} gap={4}>
      <TokenNumberInput
        token={addresses.aura}
        ticker={"AURA"}
        iconUrl={"https://www.convexfinance.com/static/icons/svg/vlcvx.svg"}
        value={auraAmount}
        onInputChange={(amount) => setAmount(amounts.map((am) => am.token == "aura" ? {token: "aura", amount} : am))}
        onBalanceRetrieval={setAuraBalance}
        onMaxClick={() => setAmount(amounts.map((am) => am.token == "aura" ? {token: "aura", amount: auraBalance.toString()} : am))}
      />
      <TokenNumberInput
        token={addresses.cvx}
        ticker={"CVX"}
        iconUrl={"https://www.convexfinance.com/static/icons/svg/vlcvx.svg"}
        value={cvxAmount}
        onInputChange={(amount) => setAmount(amounts.map((am) => am.token == "cvx" ? {token: "cvx", amount} : am))}
        onBalanceRetrieval={setCvxBalance}
        onMaxClick={() => setAmount(amounts.map((am) => am.token == "cvx" ? {token: "cvx", amount: cvxBalance.toString()} : am))}
      />
    </Flex>
  );
};
