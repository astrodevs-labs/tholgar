...
export interface WethDepositModalProps {
  step: number;
  validateStep: () => void;
}

const Swap: FC<{ validateStep: () => void }> = ({ validateStep }) => {
...
export const WethDepositModal: FC<WethDepositModalProps> = ({ step, validateStep }) => {
  const { address } = useAccount();
  if (step == 0) {
    return (
      <ApproveAllowance
    ...
    return <Swap validateStep={validateStep} />;
  }
};

WethDepositModal.defaultProps = {};
